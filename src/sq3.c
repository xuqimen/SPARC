...
void SQ3(SPARC_OBJ *pSPARC, int spn_i)
{
    ...
    if (pSPARC->ictxt_SQ > -1)
        Lanczos_dense(pSPARC->Hp_SQ, &lmin, &lmax, pSPARC->desc_Hp_SQ, 
            pSPARC->nr_Hp_SQ, pSPARC->nc_Hp_SQ, pSPARC->SQcomm, 1e-2, 1000);
    t2 = MPI_Wtime();
...
void DP_Dist2SQ(SPARC_OBJ *pSPARC)
{
    ...
    pdgemr2d_(&pSPARC->Nstates, &pSPARC->Nstates, pSPARC->Hp_cmc, &ONE, &ONE, 
              pSPARC->desc_Hp_cmc, pSPARC->Hp_SQ, &ONE, &ONE, pSPARC->desc_Hp_SQ, 
...
void DP_Project_Hamiltonian_SQ3(SPARC_OBJ *pSPARC, int *DMVertices, double *Y, int spn_i)
{
    ...
    et = MPI_Wtime();
    #ifdef DEBUG
    if (rank_kpt == 0 && spn_i == 0) printf("DP_Project_Hamiltonian, rank 0, convert BP to DP (Y) used %.3lf ms\n", 1000.0 * (et - st));
    #endif
    ...
    et = MPI_Wtime();
    #ifdef DEBUG
    if (rank_kpt == 0 && spn_i == 0) printf("DP_Project_Hamiltonian, rank 0, local dgemm for Mp used %.3lf ms\n", 1000.0 * (et - st));
    #endif
    ...
    if (rank_kpt == 0 && spn_i == 0) 
        printf("DP_Project_Hamiltonian, rank 0, reduce for Mp used %.3lf ms\n", 1000.0 * (et - st));
    #endif
    ...
    #ifdef DEBUG
    if (rank_kpt == 0 && spn_i == 0) 
        printf("DP_Project_Hamiltonian, Orthogonalization using DP used %.3lf ms\n", 1000.0 * (et - st));
    #endif
    st = MPI_Wtime();
    DP2BP(
        pSPARC->blacscomm, DP_CheFSI->nproc_row,
        ...
        DP_CheFSI->bp2dp_sendcnts, DP_CheFSI->bp2dp_sdispls,
        DP_CheFSI->dp2bp_sendcnts, DP_CheFSI->dp2bp_sdispls,
        sizeof(double), Y_dp, DP_CheFSI->Y_packbuf, pSPARC->Xorb + spn_i*size_s
    );
    et = MPI_Wtime();
    #ifdef DEBUG
    if (rank_kpt == 0 && spn_i == 0) 
        printf("DP_Project_Hamiltonian, rank 0, redist orthogonalized orbital used %.3lf ms\n", 1000.0 * (et - st));
    #endif
    ...
    Hamiltonian_vectors_mult(
        ...
        0.0, pSPARC->Xorb + spn_i*size_s, Y, pSPARC->dmcomm
    );
    et = MPI_Wtime();
    #ifdef DEBUG
    ...
    et = MPI_Wtime();
    #ifdef DEBUG
    if (rank_kpt == 0 && spn_i == 0) printf("DP_Project_Hamiltonian, rank 0, local dgemm for Hp used %.3lf ms\n", 1000.0 * (et - st));
    #endif
    ...
    #ifdef DEBUG
    if (rank_kpt == 0 && spn_i == 0) 
        printf("DP_Project_Hamiltonian, rank 0, allreduce for Hp used %.3lf ms\n", 1000.0 * (et - st));
    #endif
 ...
 * @brief   Distribute projected Hamiltonian and Density matrix
 */
void Dist2SQ(SPARC_OBJ *pSPARC)
{
#if defined(USE_MKL) || defined(USE_SCALAPACK)
    int ONE  = 1;
    
    pdgemr2d_(&pSPARC->Nstates, &pSPARC->Nstates, pSPARC->Hp, &ONE, &ONE, pSPARC->desc_Hp_BLCYC,
         pSPARC->Hp_SQ, &ONE, &ONE, pSPARC->desc_Hp_SQ, &pSPARC->ictxt_kptcomm);
    pdgemr2d_(&pSPARC->Nstates, &pSPARC->Nstates, pSPARC->Hp, &ONE, &ONE, pSPARC->desc_Hp_BLCYC,
...
/**
 * @brief   return occupation provided lambda_f when using SQ method
 */
double occ_constraint_sq(SPARC_OBJ *pSPARC, double lambda_f)
{
...
void SubDensMat(SPARC_OBJ *pSPARC, double *Ds, const double lambda_f, CHEBCOMP *cc)
{
    ...
    #else
    pdgemr2d_(&pSPARC->Nstates, &pSPARC->Nstates, Ds, &ONE, &ONE, pSPARC->desc_Ds,
         pSPARC->Mp, &ONE, &ONE, pSPARC->desc_Mp_BLCYC, &pSPARC->ictxt_kptcomm);
    #endif
...
void update_rho_psi(SPARC_OBJ *pSPARC, double *rho, const int Nd, const int Ns, const int nstart, const int nend)
{
    ...
    if(!rank) {
        #ifndef USE_DP_SUBEIG
        printf("copying orbitals psi into BLCYC format: %.3fms\n", (t2-t0)*1e3);
        printf("psi * Ds by pdgemm used: %.3fms\n", (t3-t2)*1e3);
        printf("copying result back into block format and computing rho: %.3fms\n", (t1-t3)*1e3);
        #else
        printf("psi * Ds by DP used: %.3fms\n", (t2-t0)*1e3);
        printf("Updating rho used: %3fms\n", (t1-t2)*1e3);
        #endif
        printf("Total time for updating rho and psi: %.3fms\n", (t1-t0)*1e3);
...
/**
 * @brief   Band energy using SQ method (Density matrix)
 */
double Calculate_Eband_sq(SPARC_OBJ *pSPARC, CHEBCOMP *cc)
{
    int sq3_npl, spn_i, i;
    double Eband, *Ci;
...
/**
 * @brief   Calculate electronic Entropy using SQ method (Density matrix)
 */
double Calculate_electronicEntropy_sq(SPARC_OBJ *pSPARC, CHEBCOMP *cc)
{
    int sq3_npl, spn_i, i;
    double Eent, *Ci;
